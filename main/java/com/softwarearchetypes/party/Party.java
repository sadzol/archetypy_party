...
public sealed abstract class Party permits Organization, Person {
    ...
    public Result<String, Party> add(Role role) {
        ...
        if (roleDefiningPolicy.canDefineFor(this, role)) {
            ...
            return Result.failure("POLICY_NOT_MET");
        }
    }
    ...
    public Result<String, Party> add(RegisteredIdentifier identifier) {
        checkNotNull(identifier, "Registered identifier cannot be null");

        // Check policy before adding
        if (!canRegister(identifier)) {
            return Result.failure("IDENTIFIER_NOT_ALLOWED_FOR_PARTY_TYPE");
        }
    ...
    public Result<String, Party> remove(RegisteredIdentifier identifier) {
        checkNotNull(identifier, "Registered identifier cannot be null");
        if (registeredIdentifiers.contains(identifier)) {
